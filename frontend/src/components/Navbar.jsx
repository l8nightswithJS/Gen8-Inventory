...
export default function Navbar() {
  ...
  const navLinks = (
    <>
      ...
      {isAdmin && (
        <NavLink to="/inventory/master" className={navClass} onClick={closeMenu}>
          Master View
        </NavLink>
      )}

      <NavLink to="/users" className={navClass} onClick={closeMenu}>
        Manage Users
      </NavLink>

      {isAdmin && (
        <NavLink to="/locations" className={navClass} onClick={closeMenu}>
          Manage Locations
        </NavLink>
      )}
    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 border-b border-slate-200 dark:border-slate-800 backdrop-blur shadow-sm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link
          ...
          aria-label="Gener8 Inventory clients"
        >
          <img src="/logo192.png" alt="Gener8 Logo" className="h-8 w-8" />
          <div className="font-semibold text-lg text-blue-600 dark:text-blue-400">
            Gener8{' '}
            <span className="text-slate-800 dark:text-slate-200">Inventory</span>
          </div>
        </Link>
