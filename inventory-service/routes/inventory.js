...
const { validateProfileAttributes } = require('../middleware/validateProfileAttributes');

const router = express.Router();

const optionalText = (field) =>
  body(field).optional({ nullable: true }).isString().withMessage(`${field} must be a string`);

const isSupportedDecimal = (value, { signed = false } = {}) => {
  if (value === '' || value === null || value === undefined) return true;
  const text = String(value).trim();
  const plain = signed ? /^-?\d+(?:\.\d{1,3})?$/ : /^\d+(?:\.\d{1,3})?$/;
...
const commonItemValidators = [
  ...
  body('alert_enabled').optional().isBoolean().withMessage('alert_enabled must be a boolean'),
  body('attributes').optional({ nullable: true }).isObject().withMessage('attributes must be an object'),
];

router.get('/by-location', requireRole('admin'), masterWarehouseController.getMasterInventoryByLocation);
...
router.post(
  ...
  handleValidation,
  requireItemAccess({ permission: 'edit' }),
...
router.post(
  ...
  body('to_location_id').isInt({ min: 1 }).withMessage('to_location_id is required').toInt(),
  body('from_location_id').optional({ nullable: true }).isInt({ min: 1 }).toInt(),
  body('quantity').optional({ nullable: true }).custom((value) => isSupportedDecimal(value)).withMessage('quantity must be positive with up to 3 decimals'),
  body('move_all').optional().isBoolean(),
  body('reason').optional().isString().isLength({ max: 500 }),
  handleValidation,
  requireItemAccess({ source: 'body', key: 'item_id', permission: 'edit' }),
...
router.post(
  ...
  body('remaining_quantity').custom((value) => isSupportedDecimal(value)).withMessage('remaining_quantity must be non-negative with up to 3 decimals'),
  body('reason').optional().isString().isLength({ max: 500 }),
  handleValidation,
  requireItemAccess({ permission: 'edit' }),
...
router.post(
  '/:id/quality',
  requireRole('admin', 'inventory_staff', 'project_user'),
  param('id').isInt({ min: 1 }).withMessage('Invalid id').toInt(),
  body('quality_status').isIn(['pending_inspection', 'released', 'hold', 'quarantine', 'rejected']).withMessage('Invalid quality_status'),
...
router.post(
  ...
  body('containers.*.quantity').custom((value) => isSupportedDecimal(value) && Number(String(value).replace(/,/g, '')) > 0).withMessage('each new container quantity must be greater than zero'),
...
router.post(
  ...
  requireRole('admin'),
  param('id').isInt({ min: 1 }).withMessage('Invalid id').toInt(),
  body('uom').optional({ nullable: true }).isString().isLength({ max: 40 }).withMessage('uom must be 40 characters or fewer'),
  body('allocations').isArray({ min: 1 }).withMessage('allocations must be a non-empty array'),
  body('allocations.*.location_id').isInt({ min: 1 }).withMessage('each allocation requires a valid location_id').toInt(),
  body('allocations.*.quantity').custom((value) => isSupportedDecimal(value)).withMessage('each allocation quantity must be non-negative with up to 3 decimals'),
  handleValidation,
  requireItemAccess({ permission: 'edit' }),
...
router.post(
  ...
  body('part_number').isString().trim().notEmpty().withMessage('part_number is required'),
...
router.put(
  '/:id',
  param('id').isInt({ min: 1 }).withMessage('Invalid id').toInt(),
  body('part_number').optional().isString().trim().notEmpty().withMessage('part_number cannot be empty'),
  ...commonItemValidators,
  handleValidation,
  requireItemAccess({ permission: 'edit' }),
  validateProfileAttributes,
...
router.post(
  '/adjust',
  body('item_id').isInt({ min: 1 }).withMessage('item_id is required').toInt(),
  body('location_id').isInt({ min: 1 }).withMessage('location_id is required').toInt(),
  body('change_quantity').custom((value) => isSupportedDecimal(value, { signed: true })).withMessage('change_quantity must be a number with up to 3 decimals'),
  handleValidation,
  requireItemAccess({ source: 'body', key: 'item_id', permission: 'edit' }),
...
const bulkValidators = [
  ...
  body('location_strategy').optional().isIn(['staging', 'file', 'selected']).withMessage('location_strategy must be staging, file, or selected'),
